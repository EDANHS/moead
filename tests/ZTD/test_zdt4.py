...
def test_zdt4_run_with_sbx():
    """
    Prueba ZDT4 con los operadores estándar (SBX + Polinómica)
    y la selección de padres híbrida.
    """
    print("\nIniciando test ZDT4 (con SBX + PolyMut)...")
    
    # 1. Configurar las "Estrategias"
    problem = ZDT4Problem(n_vars=10) # <-- CAMBIO
    ...
    moead_solver = MOEAD(
        ...
        n_neighbors=20,
        n_generations=400, # <-- CAMBIO (Más generaciones)
    ...
    save_path = os.path.join(IMG_DIR, "zdt4_front_sbx.png") # <-- CAMBIO
    plot_results_zdt4(pareto_front, problem, "ZDT4 con SBX", save_path) # <-- CAMBIO
    print(f"Test ZDT4 (SBX) completado. Imagen guardada en: {save_path}")

    # 4b. Guardar imagen del Historial (Análisis de Proceso)
    history_save_path = os.path.join(IMG_DIR, "zdt4_history_sbx.png") # <-- CAMBIO
    plot_history(history_log, "ZDT4 con SBX", history_save_path)
    print(f"Historial ZDT4 (SBX) guardado en: {history_save_path}")

    # 5. Verificar con Asserts (Verificación Automática)
    # Límite muy alto, esperamos que SBX falle y converja al frente falso (~1.0)
    verify_results_zdt4(pareto_front, problem, avg_error_limit=1.5) # <-- CAMBIO


# ---
# PRUEBA 2: ZDT4 con Evolución Diferencial (DE)
# ---

def test_zdt4_run_with_de():
    """
    Prueba ZDT4 con la estrategia de Evolución Diferencial (MOEA/D-DE).
    """
    print("\nIniciando test ZDT4 (con Evolución Diferencial)...")
    ...
    # 4a. Guardar imagen del Frente (Verificación Visual)
    save_path = os.path.join(IMG_DIR, "zdt4_front_de.png") # <-- CAMBIO
    plot_results_zdt4(pareto_front, problem, "ZDT4 con DE", save_path) # <-- CAMBIO
    print(f"Test ZDT4 (DE) completado. Imagen guardada en: {save_path}")

    # 4b. Guardar imagen del Historial (Análisis de Proceso)
    history_save_path = os.path.join(IMG_DIR, "zdt4_history_de.png") # <-- CAMBIO
    plot_history(history_log, "ZDT4 con DE", history_save_path)
    print(f"Historial ZDT4 (DE) guardado en: {history_save_path}")

    # 5. Verificar con Asserts (Verificación Automática)
    # Límite estricto, DE debería ser capaz de resolver esto
    verify_results_zdt4(pareto_front, problem, avg_error_limit=0.1) # <-- CAMBIO


# ---
# FUNCIONES DE AYUDA (Específicas para ZDT4)
# ---

def plot_results_zdt4(pareto_front: list, problem: Problem, title: str, save_path: str):
    """Función genérica para graficar los resultados de ZDT4."""
    
    f1_alg = [sol.objectives[0] for sol in pareto_front]
    f2_alg = [sol.objectives[1] for sol in pareto_front]

    # Generar el frente de Pareto real (Fórmula de ZDT4 es la misma que ZDT1)
    f1_real = np.linspace(0, 1, 100)
    f2_real = 1 - np.sqrt(f1_real) # <-- CAMBIO (Fórmula ZDT1/ZDT4)

    plt.figure(figsize=(10, 7))
    plt.scatter(f1_alg, f2_alg, s=10, c='blue', label='Resultado (del Archive)', alpha=0.6, zorder=2)
    plt.plot(f1_real, f2_real, 'r--', linewidth=2.5, label='Frente Real ZDT4', zorder=3) # <-- CAMBIO
    plt.title(f'Verificación del Frente de Pareto ({title})')
    plt.xlabel('Objetivo 1 (f1)')
    plt.ylabel('Objetivo 2 (f2)')
    plt.legend()
    plt.grid(True)
    plt.ylim(bottom=-0.1, top=1.5) # Muy importante para ZDT4
...
def verify_results_zdt4(pareto_front: list, problem: Problem, avg_error_limit: float):
    """Función genérica para verificar los resultados de ZDT4 con asserts."""
    ...
    for sol in pareto_front:
        ...
        # Fórmula de ZDT4 (frente real)
        f2_ideal = 1 - np.sqrt(f1_safe) # <-- CAMBIO
...
if __name__ == "__main__":
    op = 2
    if op == 1:
        test_zdt4_run_with_sbx()
    elif op == 2:
        test_zdt4_run_with_de()
    else:
        test_zdt4_run_with_sbx()
        test_zdt4_run_with_de()
