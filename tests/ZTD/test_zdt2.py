...
from moead.problems import Problem, ZDT2Problem
...
# ---
# PRUEBA 1: ZDT2 con Operadores Estándar (SBX + Mutación)
# ---

def test_zdt2_run_with_sbx():
    """
    Prueba ZDT2 con los operadores estándar (SBX + Polinómica)
    y la selección de padres híbrida.
    """
    print("\nIniciando test ZDT2 (con SBX + PolyMut)...")
    
    # 1. Configurar las "Estrategias"
    problem = ZDT2Problem(n_vars=30) # <-- CAMBIO
    scalarization = Tchebycheff()    # Tchebycheff es clave para frentes cóncavos
    ...
    save_path = os.path.join(IMG_DIR, "zdt2_front_sbx.png") # <-- CAMBIO
    plot_results_zdt2(pareto_front, problem, "ZDT2 con SBX", save_path) # <-- CAMBIO
    print(f"Test ZDT2 (SBX) completado. Imagen guardada en: {save_path}")

    # 4b. Guardar imagen del Historial (Análisis de Proceso)
    history_save_path = os.path.join(IMG_DIR, "zdt2_history_sbx.png")
    plot_history(history_log, "ZDT2 con SBX", history_save_path)
    print(f"Historial ZDT2 (SBX) guardado en: {history_save_path}")

    # 5. Verificar con Asserts (Verificación Automática)
    verify_results_zdt2(pareto_front, problem, avg_error_limit=0.1) # <-- CAMBIO


# ---
# PRUEBA 2: ZDT2 con Evolución Diferencial (DE)
# ---

def test_zdt2_run_with_de():
    """
    Prueba ZDT2 con la estrategia de Evolución Diferencial (MOEA/D-DE).
    """
    print("\nIniciando test ZDT2 (con Evolución Diferencial)...")
    ...
    # 4a. Guardar imagen del Frente (Verificación Visual)
    save_path = os.path.join(IMG_DIR, "zdt2_front_de.png") # <-- CAMBIO
    plot_results_zdt2(pareto_front, problem, "ZDT2 con DE", save_path) # <-- CAMBIO
    print(f"Test ZDT2 (DE) completado. Imagen guardada en: {save_path}")

    # 4b. Guardar imagen del Historial (Análisis de Proceso)
    history_save_path = os.path.join(IMG_DIR, "zdt2_history_de.png")
    plot_history(history_log, "ZDT2 con DE", history_save_path)
    print(f"Historial ZDT2 (DE) guardado en: {history_save_path}")

    # 5. Verificar con Asserts (Verificación Automática)
    verify_results_zdt2(pareto_front, problem, avg_error_limit=0.01) # <-- CAMBIO


# ---
# FUNCIONES DE AYUDA (Específicas para ZDT2)
# ---

def plot_results_zdt2(pareto_front: list, problem: Problem, title: str, save_path: str):
    """Función genérica para graficar los resultados de ZDT2."""
    
    f1_alg = [sol.objectives[0] for sol in pareto_front]
    f2_alg = [sol.objectives[1] for sol in pareto_front]

    # Generar el frente de Pareto real (Fórmula de ZDT2)
    f1_real = np.linspace(0, 1, 100)
    f2_real = 1 - f1_real**2 # <-- CAMBIO (Fórmula ZDT2)

    plt.figure(figsize=(10, 7))
    plt.scatter(f1_alg, f2_alg, s=10, c='blue', label='Resultado (del Archive)', alpha=0.6, zorder=2)
    plt.plot(f1_real, f2_real, 'r--', linewidth=2.5, label='Frente Real ZDT2', zorder=3) # <-- CAMBIO (Label)
...
def plot_history(history_log: dict, title: str, save_path: str):
    ...
    # Preparar datos de z_star (asumiendo 2 objetivos para ZDT2)
...
def verify_results_zdt2(pareto_front: list, problem: Problem, avg_error_limit: float):
    """Función genérica para verificar los resultados de ZDT2 con asserts."""
    ...
    for sol in pareto_front:
        ...
        f2_ideal = 1 - f1_safe**2 # <-- CAMBIO (Fórmula ZDT2)
...
if __name__ == "__main__":
    op = 3
    if op == 1:
        test_zdt2_run_with_sbx()
    elif op == 2:
        test_zdt2_run_with_de()
    else:
        test_zdt2_run_with_sbx()
        test_zdt2_run_with_de()
