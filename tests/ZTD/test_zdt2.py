...
def plot_pareto_front(pareto_front, title, filename):
    """
    Genera la validación geométrica para ZDT2.
    El frente es CÓNCAVO (f2 = 1 - f1^2).
    """
    ...
    # Generar Frente Real Teórico (ZDT2)
    x1_real = np.linspace(0, 1, 500)
    f1_real = x1_real
    f2_real = 1 - (f1_real ** 2) # Fórmula Cóncava
    ...
    plt.plot(f1_real, f2_real, 'r-', linewidth=2.5, label='Frente Teórico (ZDT2)', zorder=1)
    
    plt.title(f'Validación de Frente Cóncavo: {title}')
...
def calculate_metrics_zdt2(pareto_front):
    """
    KPIs específicos para ZDT2.
    """
    if not pareto_front:
        return 1.0, 0.0
    ...
    for sol in pareto_front:
        ...
        # Frente ideal ZDT2: f2 = 1 - f1^2
        f1_safe = max(f1, 0.0)
        f2_ideal = 1 - (f1_safe ** 2)
...
def run_experiment_zdt2(algorithm_type):
    """
    Orquestador para ZDT2.
    """
    problem = ZDT2Problem(n_vars=30) 
    
    if algorithm_type == 'SBX':
        print("\n--> Iniciando ZDT2 (Cóncavo) con MOEA/D Estándar (SBX)...")
        ...
        print("\n--> Iniciando ZDT2 (Cóncavo) con MOEA/D Evolución Diferencial (DE)...")
    ...
    solver = MOEAD(
        problem=problem,
        scalarization=Tchebycheff(), # Tchebycheff es mejor para frentes cóncavos que PBI
    ...
    # Visualización
    plot_pareto_front(pareto_front, f"ZDT2 - {label}", f"zdt2_front_{file_suffix}.png")
    plot_convergence_history(history, label, f"zdt2_history_{file_suffix}.png")
    
    # Métricas
    error, spread = calculate_metrics_zdt2(pareto_front)
...
def generate_comparative_charts_zdt2(sbx_metrics, de_metrics):
    """
    Genera gráficos comparativos.
    """
    print("\n--> Generando gráficos comparativos de alto nivel para ZDT2...")
    ...
    plt.title('Precisión de Convergencia (ZDT2 - Cóncavo)', fontsize=14, fontweight='bold')
    ...
    plt.savefig(os.path.join(IMG_DIR, "zdt2_comparison_error.png"))
    plt.close()

    # Gráfico B: Cobertura / Spread
    plt.figure(figsize=(8, 6))
    bars = plt.bar(strategies, spreads, color=colors, width=0.5, edgecolor='black', alpha=0.9)
    plt.title('Diversidad en Geometría Cóncava (Spread)', fontsize=14, fontweight='bold')
    ...
    plt.savefig(os.path.join(IMG_DIR, "zdt2_comparison_spread.png"))
    plt.close()
...
if __name__ == "__main__":
    # 1. Ejecutar Baseline
    metrics_sbx = run_experiment_zdt2('SBX')
    
    # 2. Ejecutar Propuesta
    metrics_de = run_experiment_zdt2('DE')
    
    # 3. Sintetizar Comparación
    generate_comparative_charts_zdt2(metrics_sbx, metrics_de)
    
    print("\n¡Material para ZDT2 (Cóncavo) generado exitosamente!")